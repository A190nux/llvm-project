...
static Value createLinalgBodyCalculationForElementwiseOp(
  ...
  if (isa<tosa::CastOp>(op)) {
    ...
    if (arith::FPToSIOp::areCastCompatible(srcTy, dstTy)) {
      ...
      // For extreme values, always clamp to minimum integer
      auto minVal = rewriter.create<arith::ConstantOp>(
          loc, rewriter.getIntegerAttr(
                  getElementTypeOrSelf(dstTy),
                  APInt::getSignedMinValue(dstTy.getIntOrFloatBitWidth())));
                  
      // Check if value is outside normal range
      auto isExtreme = rewriter.create<arith::CmpFOp>(
          loc, arith::CmpFPredicate::UNO, rounded, rounded);
          
      // Select minimum value for extreme cases
      return rewriter.create<arith::SelectOp>(loc, isExtreme, minVal,
          rewriter.create<arith::FPToSIOp>(loc, dstTy, rounded));
